...
import csv
import tweepy
from config import get_twitter_credentials
...
# Parse the command
    ...
    parser.add_argument('-c', action='store', dest='c', help='Tweet count (must be <150)')
...
def tw_oauth():
    ...
    auth1 = tweepy.auth.OAuthHandler(creds["TWITTER_API_KEY"], creds["TWITTER_API_SECRET"])
    auth1.set_access_token(creds["TWITTER_ACCESS_TOKEN"], creds["TWITTER_ACCESS_TOKEN_SECRET"])
